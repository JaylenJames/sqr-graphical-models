		...
		uint32_t* iter, double* dGap)
{
	...
	for (; NewtonIter <= maxNewtonIter; NewtonIter++) {
		...
		// First step do something different
		if (NewtonIter == 1 && IsDiag(p, X)) {
			...
			for (unsigned long cdSweep = 1; cdSweep <= 1 + NewtonIter/3;
				...
				// Random permutation over free set
		...
		// Line search using approximate Newton direction
		double alpha = 1.0;
		double l1normXD = 0.0;
		double fX1prev = 1e+15;
		for (unsigned long lineiter = 0; lineiter < max_lineiter;
				lineiter++) {
			// Computing L1 and trace term
			double l1normX1 = 0.0;
			double trSX1 = 0.0;
			for (unsigned long i = 0, k = 0; i < p ; i++, k += p) {
				for (unsigned long j = 0; j < i; j++) {
			...
			// Computing Cholesky factorization in order for log det(X)
			// Note that the upper triangular part is the lower
			// triangular part for the C arrays.
			ptrdiff_t info = 0;
			ptrdiff_t p0 = p;
			...
			// Sanity check for not decreasing
			if (fX1prev < fX1) {
				fXprev = fX;
				l1normX = l1normX1;
				logdetX = logdetX1;
		...
		// W is covariance matrix from the precision matrix X
		// compute W = inv(X):
		// Compute from Cholesky factorization (faster than directly computing)
		ptrdiff_t info;
		ptrdiff_t p0 = p;
		...
		if (subgrad*alpha >= l1normX*tol && (fabs((fX - fXprev)/fX) >= EPS))
			continue;
		// Set for path of lambda
		// intialize smaller lambda problem with previous lambda
		if (mode =='P') {
			if (opt != NULL)
				opt[pathIdx] = fX;
			if (iter != NULL)
	...
	// Store objective function for each iteration
	if (mode == 'T' && iter != NULL)
		iter[0] = NewtonIter - 1;
	free(activeSet);
	free(U);
